...
function selectAllTema() {
  // temaClassのものを全て取得し、すべてのチェックを入れる
  document.querySelectorAll('.tema').forEach(cb => cb.checked = true);
}

// 手間チェックボックス選択全解除
function clearAllTema() {
  // temaClassのものを全て取得し、すべてのチェックを外す
  document.querySelectorAll('.tema').forEach(cb => cb.checked = false);
}

// ご飯かお菓子かの選択
function selectMenu(value, btn) {
  // 隠されている場所にあるinputのmenuFlagを取得し、受け取ったValueをセットする
  document.getElementById('menuFlag').value = value;
...
function loadIngredients(menuFlag) {
  ...
  const filtered = allData.filter(row => {
    if (menuFlag === '1') return row.isGohan === true;
    if (menuFlag === '0') return row.isOkashi === true;
    return false;
  });

  // ジャンルの重複を除いてプルダウンに追加
  const genres = [...new Set(filtered.map(row => row.genre).filter(Boolean))].sort();
...
function submit() {
  // menuFlagを取得
  const menuFlag = document.getElementById('menuFlag').value;
  // フラグがない場合はエラーを出力
  if (menuFlag === '') {
  ...
  const filtered = allData.filter(row => {
    ...
    if (ingredientFilter !== 'all' && row.genre !== ingredientFilter) return false;
    // 除外されていないデータを返す
    return true;
  });
...
function showResult(result) {
  ...
  resultDiv.innerHTML =
    `今日の${menuname}は「<strong>${result.name}</strong>」です！
     <br><br>${result.site}<br>
     <a href="${result.url}" target="_blank">${result.url}</a>
     <img id="ogpImage" src="" alt="画像" style="display:none; width:100%; border-radius:8px; margin-top:12px;">`;

    // OGP画像取得
  if (result.url && result.url !== 'URL無し') {
    ...
    fetch(`${gasUrl}?url=${encodeURIComponent(result.url)}`)
      .then(res => res.json())
      .then(ogp => {
        if (ogp.success) {
          const img = document.getElementById('ogpImage');
          img.src = ogp.imageUrl;
